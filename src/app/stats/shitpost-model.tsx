...
export default function extrapolate(
  ...
  let releases_extrapolated = extrapolateSeries(
    years,
    values.map((el) => el.new_releases),
  );
  let files_extrapolated = extrapolateSeries(
    years,
    values.map((el) => el.total_files),
  );
  let size_extrapolated = extrapolateSeries(
    years,
    values.map((el) => el.total_size),
  );
...
function extrapolateSeries(years: number, values: number[]) {
  let time_slice = cumulative_sum(
    values.slice(values.length - 12, values.length).map((x) => ({ x })),
    ["x"],
  ).map(({ x }) => x);
