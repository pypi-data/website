...
export default async function Page() {
    ...
    const sqliteSchemaResponse = await fetch("https://raw.githubusercontent.com/pypi-data/pypi-json-data/main/scripts/schema.sql");
    const sqliteSchema = await sqliteSchemaResponse.text();

    const sqliteSizes = [
        {url: SQLITE_URL, size: sqliteSize}
    ]
    ...
    return (
        <>
            ...
            <article className="prose">
                ...
                <p>
                    These datasets allow you to analyse the contents of PyPI without having to download and process
                    every package yourself. All of the statistics within the <a href="/website/stats">stats page</a>
            ...
            <div className="card rounded-box bg-base-100 shadow-2xl">
                <div className="card-body">
                    <h1 id="metadata" className="card-title">Metadata about every file uploaded to PyPI</h1>
                    <h3>About</h3>
                    <article className="prose lg:prose-md">
                        ...
                        <p>
                            The dataset should be accessed by downloading the files specified
                            within <a href={DATASET_URL} target="_blank"><code>{DATASET_URL}</code></a> .
                        ...
                        <details className="collapse bg-base-200 mb-5 mt-5">
                            ...
                            <div className="collapse-content">
                                <p className="mt-0">
                                    <a href="https://duckdb.org/">DuckDB</a> is a great tool for processing the dataset.
                                    It is very fast and supports SQL queries over Parquet files. The following
                                    command uses DuckDB to find the largest file ever uploaded to PyPI:
                                </p>
                                <SyntaxHighlight language="shell">
                                    {DUCK_DB_EXAMPLE}
                                </SyntaxHighlight>
                        ...
                        <details className="collapse bg-base-200 mb-5 mt-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="ml-5">
                                    <p className="mt-0">
                                        The dataset contains a <code>skip_reason</code> column. If a file is not present
                                        in the git
                                        repositories
                                        then
                                        the reason for skipping is recorded here. On the right is a list of the current
                                        skip reasons and the
                                        number of
                                        files excluded from the git repositories for each reason.
                                    </p>
                                    <p>
                                        The exact reasons for skipping a file are not fully documented here,
                                        but <code>ignored</code> files
                                        include
                                        virtual environments accidentally uploaded to
                                        PyPI. <code>text-long-lines</code> means the file had
                                        very
                                        few lines, but the total size was large.
                                    </p>
                                </div>
                                <div>
                                    <strong>Skipped reasons:</strong>
                                    <Table data={stats.skip_reason_stats.filter(({skip_reason}) => skip_reason != "")}
                                           columns={[{name: "skip_reason"}, {name: "count", type: "number"}, {name: "total_size", type: "bytes"}]}/>
                                </div>
                            </div>
                        </details>
                    </article>
            ...
            <div className="card rounded-box bg-base-100 shadow-2xl">
                <div className="card-body">
                    <h1 id="sqlite-dump" className="card-title">SQLite dump of all PyPI metadata</h1>
                    <h3>About</h3>
                    <p>
                        This is a SQLite dump of all PyPI metadata fetched from the PyPI API. It is updated
                        daily. It can be accessed directly from the following url:&nbsp;
                        <a href={SQLITE_URL} target="_blank"><code>{SQLITE_URL}</code></a>:
                    </p>
                    <SyntaxHighlight language="shell">
                        {SQLITE_CURL_EXAMPLE}
                    </SyntaxHighlight>
