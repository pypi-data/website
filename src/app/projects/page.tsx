  ...
  return (
    <>
      <article className="prose lg:prose-md mb-3">
        <h1>
          Projects List <small>{data.packages.length.toLocaleString()} projects</small>
        </h1>
      </article>
      <form
        className="w-full"
            ...
            {searchResults.map((p) => {
              return (
                <tr key={p}>
                  <th>
                    <p>
                      <a href={`/website/projects/view?name=${p}`}>{p}</a>
                    </p>
                  </th>
                  <td>
                    <a role={"button"} className={"btn btn-info btn-xs"} href={`/website/projects/view?name=${p}`}>
                      View
                    </a>
                  </td>
                </tr>
